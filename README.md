...
当前版本 `0.1.24-candidate` 是尚未批准的本地修订候选，
...
codex plugin marketplace add ifoohoo/skill-family-audit \
  --ref skill-family-audit-v0.1.24-candidate --json
...
```text
dist/candidate/0.1.24-candidate/platforms/<platform>
```

其中 Codex 投影通过本地 marketplace 在独立 `CODEX_HOME` 中验证完整功能
生命周期；Claude Code、Kimi Code 0.31.0 与 WorkBuddy 分别在独立配置根中
验证持久安装、发现、最小调用和功能卸载。后三个平台的原生即时物理清理，
以及四个平台的升级、回滚和公开发布仍未验证。
...
本候选在 Codex 中完成安装、发现、最小调用、卸载和清理闭环；Claude Code、
Kimi Code 与 WorkBuddy 完成功能生命周期，但原生即时物理清理仍受阻。
不得把候选批准或功能生命周期成功解释为四平台稳定支持。
