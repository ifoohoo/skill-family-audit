...
let value;
if (input.operation === "digest-document") {
  value = { digest: runner.digestDocument(input.document) };
} else if (input.operation === "canonical-json") {
  value = { text: runner.canonicalJson(input.document) };
} else if (input.operation === "resource-closure") {
  value = await runner.computeResourceClosure({
    root: input.root,
    resources: input.resources,
  });
} else {
  throw new Error(`unknown Foundation operation: ${input.operation}`);
}
...
def verify_foundation_bundle(runner_ref: str) -> Path:
    """Bind a runner reference to its managed Bundle; reject hostile twins."""
    raw = Path(runner_ref).expanduser()
    if raw.is_symlink():
        raise RuntimeError("FOUNDATION_RUNNER_SYMLINK")
    if not raw.is_file() or raw.name != "runner.mjs":
        raise RuntimeError("FOUNDATION_RUNNER_INVALID")
    ...
    for member in (provenance_raw, validators_raw):
        if member.is_symlink():
            raise RuntimeError("FOUNDATION_BUNDLE_MEMBER_SYMLINK")
    ...
    if (
        ...
        raise RuntimeError("FOUNDATION_BUNDLE_SOURCE_INVALID")
    ...
    if actual != set(declared) | {"foundation-projection.json"}:
        raise RuntimeError("FOUNDATION_BUNDLE_MEMBERSHIP_INVALID")
    ...
    if receipt_raw.is_file():
        ...
        if (
            ...
            or receipt.get("provenanceDigest") != _sha256(provenance_raw)
        ):
            raise RuntimeError("FOUNDATION_RECEIPT_INVALID")
    return runner
...
def foundation_runner() -> Path:
    configured = os.environ.get("SFA_FOUNDATION_RUNNER_REF")
    if configured:
        return verify_foundation_bundle(configured)
    for ancestor in Path(__file__).resolve().parents:
        candidate = ancestor / "foundation/quickstart-profile/runner.mjs"
        if candidate.is_file() or candidate.is_symlink():
            return verify_foundation_bundle(str(candidate))
    raise RuntimeError(
        "FOUNDATION_RUNNER_MISSING: 需要受管 Foundation Quickstart Profile v2 Bundle"
    )
...
def _foundation(request: dict) -> dict:
    ...
    completed = subprocess.run(
        ...
        input=json.dumps({**request, "runner": str(runner)}, ensure_ascii=False),
        capture_output=True,
        text=True,
        check=False,
    )
