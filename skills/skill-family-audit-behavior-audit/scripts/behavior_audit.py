...
def _install_foundation_host() -> None:
    ...
    if script is None:
        raise BehaviorCliError("FOUNDATION_TRANSPORT_MISSING")
...
def implementation_digest() -> str:
    ...
    shared_content = (scripts_dir.parents[2] / shared_relative).read_bytes()
    digest.update(shared_relative.encode("utf-8"))
    digest.update(b"\0")
    digest.update(hashlib.sha256(shared_content).digest())
