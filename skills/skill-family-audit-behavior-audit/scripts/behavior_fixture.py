...
BEHAVIOR_FIXTURE_SCHEMA_ID = "spec/contracts/behavior-fixture-manifest.schema.json"
...
def _resolve_contained(
    ...
    *,
    ...
    """根内路径委托 Foundation resolve-contained 做收容检查；root 必填，无本地回退。"""
    try:
        ...
        errors.append(f"{label}: Foundation containment rejected {path} — {exc}")
        return None
    try:
...
def _resolve_no_symlink(
    path: Path,
    label: str,
    errors: list[str],
) -> Path | None:
    """根级绝对路径的零符号链接解析；明确无 root 收容语义（收容由调用方显式判定）。"""
    try:
...
def _contains(path: Path, root: Path) -> bool:
    """path 是否在 root 内（等根视为在内）；判定委托 Foundation resolve-contained。"""
    if path == root:
        return True
    try:
        _foundation({
...
def _foundation_file_digest(path: Path) -> str:
    result = _foundation({
        "operation": "resource-closure",
        "root": str(path.parent),
        "resources": [{"path": path.name, "role": "input"}],
    })
    return str(result["resources"][0]["sha256"])
...
def validate_fixture_schema(
    ...
    """通过受管 Foundation Bundle 校验夹具清单。"""
    if schema_name != "behavior-fixture-manifest":
        raise KeyError(f"unknown schema: {schema_name}")
    result = _foundation({
        "operation": "validate-by-schema-id",
        "schemaId": BEHAVIOR_FIXTURE_SCHEMA_ID,
        "document": instance,
    })
    return [] if result.get("valid") is True else [str(e) for e in result.get("errors", [])]
    ...
    *,
    ...
    for idx, fixture in enumerate(fixtures):
        ...
        if not exe.is_absolute():
            ...
            exe_resolved = _resolve_contained(
                exe, f"{prefix} ({fid}) argv[0]", errors, root=sandbox_root
            )
            if exe_resolved is not None:
                if not exe_resolved.is_file():
                    ...
                    actual_digest = _foundation_file_digest(exe_resolved)
        ...
        cwd_resolved = _resolve_contained(
            cwd_path, f"{prefix} ({fid}) cwd", errors, root=sandbox_root
        )
        ...
        for ws_rel in write_set:
            ...
            ws_resolved = _resolve_contained(
                ws_path,
                f"{prefix} ({fid}) write_set '{ws_rel}'",
                errors,
                root=cwd_resolved or sandbox_root,
            )
        ...
        for of in output_files:
            ...
            _resolve_contained(
                of_path,
                f"{prefix} ({fid}) output_file '{of_rel}'",
                errors,
                root=cwd_resolved or sandbox_root,
            )
...
def load_and_validate_fixture_plan(
    *,
    ...
    if _contains(sandbox_resolved, target_resolved):
        errors.append("sandbox_root is inside target_project_path")
    if _contains(target_resolved, sandbox_resolved):
    ...
    schema_errors = validate_fixture_schema(manifest_raw, "behavior-fixture-manifest")
    ...
    proof_resolved = _resolve_contained(
        proof_path, "provider proof_path", errors, root=sandbox_resolved
    )
    ...
    receipt_resolved = _resolve_contained(
        receipt_path, "authorization receipt_path", errors, root=sandbox_resolved
    )
    ...
    actual_proof_digest = _foundation_file_digest(proof_resolved)
    ...
    actual_receipt_digest = _foundation_file_digest(receipt_resolved)
    ...
    # ------------------------------------------------------------------
    # 消费调用方给出的隔离事实；Audit 不拥有通用隔离 Schema。
    # ------------------------------------------------------------------
    ...
    # manifest 摘要
    manifest_digest = _foundation_file_digest(manifest_resolved)
