...
def _assessment_rows(value: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    rows = [
        (name, row)
        for name, row in value.get("assessments", {}).items()
        if name != "platforms" and isinstance(row, dict)
    ]
    rows.extend(
        (f"platform:{name}", row)
        for name, row in value.get("assessments", {})
        .get("platforms", {})
        .items()
...
def _assessment_evidence_refs(
    *,
    ...
    refs = []
    for offset, (name, row) in enumerate(_assessment_rows(assessment_result)):
        path = row.get("path")
        digest = row.get("digest")
        if not isinstance(path, str) or not isinstance(digest, str):
            continue
        refs.append({
            "evidence_id": f"release-assessment:{offset}:{name}",
            "kind": "execution_record",
            "path": _require_absolute(path, f"assessment.{name}.path"),
            "source": _require_text(
                row.get("evidence_type"), f"assessment.{name}.evidence_type"
            ),
            "candidate_id": candidate_id,
            "task_id": run_id,
            "environment_digest": environment_digest,
            "rule_ref": f"release-assessment:{name}",
            "implementation_digest": implementation_digest,
            "executor_id": _require_text(
                row.get("identity"), f"assessment.{name}.identity"
            ),
            "sequence_number": start_sequence + offset,
            "content_digest": _require_digest(
                digest, f"assessment.{name}.digest"
            ),
            "credibility": "verified",
        })
    return refs
...
def build_public_bundle(
    *,
    ...
    if (assessment_result is None) != (assessments_path is None):
        raise ReleasePublicError(
            ["assessment_result 与 assessments_path 必须同时提供"]
        )
    assessment_status = (
        "SUCCEEDED"
    ...
    if assessments_path is not None:
        assessments_path = _require_absolute(
            assessments_path, "assessments_path"
        )
        evidence.extend(
            _assessment_evidence_refs(
                assessment_result=assessment_result or {},
                run_id=run_id,
                candidate_id=candidate_id,
                environment_digest=environment_digest,
                implementation_digest=implementation_digest,
                start_sequence=len(evidence),
            )
        )
    ...
    assessment_blockers = [
        ...
        if row.get("status")
        not in {"valid", "exception_pass", "not_applicable"}
    ]
    ...
    domain_result = {
        ...
        "protocol_version": "1.1.0-candidate",
        "status": status,
    ...
    method_values = {
        "release-result.json": domain_result,
    ...
    if assessments_path is not None:
        input_resources.append(
            _resource(
                resource_id="release-assessments-input",
    ...
    task = {
        ...
        "parameters": {
            ...
            "assessments_ref": assessments_path or "",
        },
    ...
    result = {
        ...
        "summary": {
            "SUCCEEDED": "Release Skill 收据链与全部审计评估均通过",
            "BLOCKED": "Release Skill 收据链或审计评估尚未满足",
            "FAILED": "Release Skill 收据链或审计评估无效",
        }[status],
        "outputs": [
        ...
        "evidence": evidence,
        "missing_inputs": (
            blocking_reasons if status == "BLOCKED" else []
        ),
        "warnings": [],
        ...
        "extensions": [{
            "receipt_audit_status": recomputed["status"],
