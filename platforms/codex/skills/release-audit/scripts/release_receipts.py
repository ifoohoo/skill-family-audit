"""Release Skill 0.2.7 immutable receipt-chain validator.
...
def _canonical_json(value: Any) -> str:
    """Match Release Skill canonicalJson for JSON-compatible values."""
    return json.dumps(
        value,
        ensure_ascii=False,
        allow_nan=False,
...
def _compute_plan_digest(plan: dict[str, Any]) -> str:
    ...
    return _sha256(_canonical_json(binding).encode("utf-8"))
...
def _validate_plan(
    ...
    *,
    ...
    if plan.get("planVersion") != 2:
        raise ReleaseReceiptError("release-audit 仅接受 Release Skill 0.2.7 planVersion 2")
...
def _validate_source_authority_binding(plan: dict[str, Any]) -> None:
    ...
    if not authority:
        raise ReleaseReceiptError("0.2.7 生产计划缺少冻结 sourceAuthority")
...
def _validate_skill_resource_plan_binding(plan: dict[str, Any]) -> None:
    ...
    if not closure:
        raise ReleaseReceiptError("0.2.7 生产计划缺少 skillResourceClosure")
        ...
        if terminal_run.get("status") == "VERIFIED":
            ...
            for gate_id, gate in consumer_gate_map.items():
                ...
                expected_digest = _sha256(
                    _canonical_json(gate).encode("utf-8")
                )
                if (
