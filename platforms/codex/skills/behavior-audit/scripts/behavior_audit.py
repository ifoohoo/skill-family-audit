...
_QUICKSTART_CONTRACTS: Any = None
...
def _consume_quickstart_task(argv: list[str]) -> dict[str, Any] | None:
    global _QUICKSTART_CONTRACTS
    if not os.environ.get("SFA_NORMALIZED_TASK_REF"):
        return None
    path = Path(os.environ.get("SFA_RUNTIME_CONTRACTS_REF", ""))
    if not path.is_absolute() or path.is_symlink():
        raise BehaviorCliError("normalized Task contracts ref is invalid")
    resolved = path.resolve(strict=True)
    spec = importlib.util.spec_from_file_location(
        "behavior_quickstart_contracts", resolved
    )
    if spec is None or spec.loader is None:
        raise BehaviorCliError("normalized Task contracts are unavailable")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    _QUICKSTART_CONTRACTS = module
...
def run(
    ...
    if quickstart_binding is not None:
        ...
        behavior_contracts.assert_valid_envelope(
            bundle["task"], bundle["result"], bundle["evidence_items"]
        )
    ...
    artifact_digests = behavior_publish.publish_artifacts(
        output_dir=bound_context["output_dir"],
        artifact_bytes=bundle["artifact_bytes"],
    )
    execution_status = bundle["result"]["execution_status"]
    response = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        quickstart_binding = _consume_quickstart_task(raw_argv)
        exit_code, response = run(
            _parser().parse_args(raw_argv), quickstart_binding
        )
    except Exception as exc:
        errors_list = getattr(exc, "errors", [str(exc)])
        run_id = "run-behavior-audit-unknown"
        try:
        ...
        error_obj = behavior_contracts.build_error(
            error_code="BEHAVIOR_AUDIT.EXECUTION_EXCEPTION",
            category="EXECUTION_EXCEPTION",
            stage_id="behavior-audit",
            message=str(exc),
            affected_resources=[],
            evidence=[],
        ...
        if quickstart_binding is not None:
            result_envelope.setdefault("extensions", []).append(quickstart_binding)
            _QUICKSTART_CONTRACTS.publish_bound_failure_result(
                quickstart_binding, result_envelope
            )
