...
MATURITY_LEVELS: dict[str, dict[str, Any]] = {
    "candidate_ready": {
        "required_artifacts": {
            ...
            "observer_events": {
                "schema_source": "spec",
                "schema_filename": "observer-event.schema.json",
            },
        },
    },
    "consumer_qualified": {
...
_CONTRACT_DIGEST_KEYS = [
    ...
    "spec:observer-event.schema.json",
]
...
def load_loop_provider_contract(
    ...
    # ---- 公共 Schema 根 ----
    if not os.path.isdir(public_contract_root):
        raise RuntimeEvidenceValidationError(
            [f"public_contract_root does not exist: {public_contract_root}"]
        )

    obs_path = os.path.join(public_contract_root, "observer-event.schema.json")
    obs_key = "spec:observer-event.schema.json"
    if not os.path.isfile(obs_path):
        errors.append("observer-event.schema.json does not exist in public_contract_root")
    elif os.path.islink(obs_path):
        errors.append("observer-event.schema.json is a symlink")
    else:
        with open(obs_path, "rb") as f:
            raw_bytes = f.read()
        contracts[obs_key] = hashlib.sha256(raw_bytes).hexdigest()
        _schema_cache[obs_key] = json.loads(raw_bytes)
...
def _validate_observer_events_array(
    artifact: Any,
    evidence_root: str,
    provider_contracts: dict[str, str],
    category_spec: dict[str, str],
) -> tuple[list[dict[str, Any]] | None, list[str]]:
    """验证 observer_events：路径安全、存在、JSON 有效、摘要匹配、Schema 校验、非空数组。"""
    errors: list[str] = []
    prefix = "/artifacts/observer_events"
    ...
    # schema_source / schema_filename 基本格式（纠偏 #3：与普通 artifact 相同检查）
    if not isinstance(schema_source, str) or schema_source not in ("loop-agent", "spec"):
        errors.append(
            f"{prefix}/schema_source: must be 'loop-agent' or 'spec', got {schema_source!r}"
        )
    ...
    # schema_digest 匹配 provider_contracts（纠偏 #3：与普通 artifact 相同检查）
    contract_key = f"{schema_source}:{schema_filename}"
    expected_sd = provider_contracts.get(contract_key)
    if expected_sd is None:
        if schema_source in ("loop-agent", "spec") and schema_filename:
    ...
    if not isinstance(content, list) or len(content) == 0:
        errors.append(f"{prefix}/content: must be a non-empty array")
        return None, errors

    # 每项独立 Schema 校验
    schema_key = contract_key
    if schema_key in _schema_cache:
        for idx, event in enumerate(content):
            ev_errors = _validate_json_object(
                event, schema_key, f"{prefix}/content[{idx}]"
            )
...
def _validate_trusted_producer_receipt(
    bundle: dict[str, Any], provider_root: str
) -> list[str]:
    """只信任冻结档案列出的 provider 产出收据，不信任 caller 声明。"""

    descriptor = bundle.get("producer_receipt")
    if not isinstance(descriptor, dict):
        return ["/producer_receipt: trusted provider receipt unavailable"]
    if set(descriptor) != {"path", "content_digest"}:
        return [
            "/producer_receipt: must contain only path and content_digest"
        ]
    relative = descriptor.get("path")
    claimed_digest = descriptor.get("content_digest")
    if (
        ...
        return ["/producer_receipt: invalid descriptor"]
    ...
    try:
        ...
        return ["/producer_receipt/path: escapes provider root"]
    if candidate.is_symlink() or not resolved.is_file():
        return ["/producer_receipt: trusted provider receipt unavailable"]
    raw = resolved.read_bytes()
    actual_digest = _compute_sha256(raw)
    if actual_digest != claimed_digest:
        return [
            "/producer_receipt/content_digest: mismatch: "
            f"expected {actual_digest}, got {claimed_digest}"
        ]
    try:
        profile = json.loads(_EVIDENCE_PROFILE.read_text(encoding="utf-8"))
        receipt = json.loads(raw)
    except (OSError, json.JSONDecodeError) as exc:
        return [f"/producer_receipt: invalid JSON: {exc}"]
    trusted = profile.get("trusted_producer_receipt_digests", [])
    if actual_digest not in trusted:
        return ["/producer_receipt: trusted provider receipt unavailable"]
    if not isinstance(receipt, dict):
        return ["/producer_receipt: receipt must be an object"]
    unsigned_bundle = dict(bundle)
    unsigned_bundle.pop("producer_receipt", None)
    expected_bundle_digest = _compute_sha256(
        _canonical_json(unsigned_bundle).encode("utf-8")
    )
    provider = bundle.get("provider", {})
    if (
        receipt.get("receipt_kind") != "runtime-provider-production-receipt"
        or receipt.get("provider_id") != provider.get("provider_id")
        or receipt.get("provider_version") != provider.get("provider_version")
        or receipt.get("bundle_digest") != expected_bundle_digest
    ):
        return ["/producer_receipt: identity or bundle binding mismatch"]
    return []
...
def _validate_artifacts(
    ...
    for cat, spec in required_artifacts.items():
        ...
        if cat == "observer_events":
            content, cat_errors = _validate_observer_events_array(
                artifacts[cat], evidence_root, provider_contracts, spec
            )
        else:
...
def _validate_observer_identity_and_chain(
    ...
    errors: list[str] = []
    identity = bundle.get("identity", {})
    process_evidence = loaded.get("process_evidence", {})

    bundle_run_id = identity.get("run_id")
    bundle_task_id = identity.get("task_id")
    pe_process_id = process_evidence.get("process_id")

    for idx, event in enumerate(events):
        ev_run_id = event.get("run_id")
        ev_task_id = event.get("task_id")
        ev_process_id = event.get("process_id")
        ev_seq = event.get("sequence_number")

        if ev_run_id != bundle_run_id:
            errors.append(
                f"observer[{idx}]/run_id={ev_run_id!r} != identity/run_id={bundle_run_id!r}"
            )
        if ev_task_id != bundle_task_id:
            errors.append(
                f"observer[{idx}]/task_id={ev_task_id!r} != identity/task_id={bundle_task_id!r}"
            )
        if ev_process_id != pe_process_id:
            errors.append(
                f"observer[{idx}]/process_id={ev_process_id!r} "
                f"!= process_evidence/process_id={pe_process_id!r}"
            )

        # sequence_number 严格递增且不得重复
        if idx > 0:
            prev_seq = events[idx - 1].get("sequence_number")
            if not (isinstance(ev_seq, int) and isinstance(prev_seq, int) and ev_seq > prev_seq):
                errors.append(
                    f"observer[{idx}]/sequence_number={ev_seq!r} is not strictly greater "
                    f"than previous={prev_seq!r}"
                )

            # previous_digest 链
            prev_event_digest = events[idx - 1].get("event_digest")
            cur_prev_digest = event.get("previous_digest")
            if cur_prev_digest != prev_event_digest:
                errors.append(
                    f"observer[{idx}]/previous_digest={cur_prev_digest!r} "
                    f"!= observer[{idx - 1}]/event_digest={prev_event_digest!r}"
                )
        else:
            # 首项 previous_digest 必须为 null
            if event.get("previous_digest") is not None:
                errors.append(
                    f"observer[0]/previous_digest must be null, "
                    f"got {event.get('previous_digest')!r}"
                )
        declared_event_digest = event.get("event_digest")
        event_payload = {
            key: value
            for key, value in event.items()
            if key != "event_digest"
        }
        actual_event_digest = _compute_sha256(
            _canonical_json(event_payload).encode("utf-8")
        )
        if declared_event_digest != actual_event_digest:
            errors.append(
                f"observer[{idx}]/event_digest={declared_event_digest!r} "
                f"!= canonical event digest {actual_event_digest!r}"
            )
...
def _validate_bundle(
    ...
    *,
    ...
    # ---- 5. 观察事件身份与链（仅对已成功加载的 category 执行） ----
    observer_events = loaded.get("observer_events")
    if observer_events is not None and "process_evidence" in loaded:
        obs_errors = _validate_observer_identity_and_chain(
            observer_events, bundle, loaded
        )
        errors.extend(obs_errors)

    # ---- 6. Stable 额外要求（不因 artifact 失败跳过，纠偏 #8） ----
...
def validate_runtime_evidence_bundle(
    ...
    *,
    ...
    receipt_errors = _validate_trusted_producer_receipt(
        bundle, provider_root
    )
    return sorted(profile_errors + receipt_errors + _validate_bundle(
        bundle,
