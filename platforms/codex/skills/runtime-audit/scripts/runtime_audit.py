...
_BLOCKED_PATTERNS = (
    ...
    "trusted provider receipt unavailable",
)
...
class RuntimeAuditError(RuntimeError):
    """预执行验证失败，可被 main 捕获并转为 Result 信封。"""
...
def validate_absolute_path(value: str, label: str) -> Path:
    ...
    try:
        ...
        raise RuntimeAuditError(f"{label} 路径解析失败: {e}") from e
    return p
...
def validate_dir_not_symlink(path: Path, label: str) -> None:
    """校验目录真实存在，且根不得为符号链接。"""
    if path.is_symlink():
        raise RuntimeAuditError(f"{label} 不得是符号链接: {path}")
    if not path.is_dir():
        raise RuntimeAuditError(f"{label} 必须是存在的目录: {path}")
...
def validate_file_not_symlink(path: Path, label: str) -> None:
    ...
    if path.is_symlink():
        raise RuntimeAuditError(f"{label} 不得是符号链接: {path}")
    if not path.is_file():
        raise RuntimeAuditError(f"{label} 必须是存在的普通文件: {path}")
...
def validate_output_dir(path: Path) -> None:
    ...
    if path.exists():
        if not path.is_dir():
            raise RuntimeAuditError(f"output-dir 已存在但不是目录: {path}")
        if any(path.iterdir()):
            raise RuntimeAuditError(f"output-dir 必须不存在或为空: {path}")
    parent = path.parent
    if not parent.is_dir():
        raise RuntimeAuditError(f"output-dir 父目录必须是存在的目录: {parent}")
...
def main() -> None:
    ...
    quickstart_binding = None
    try:
        quickstart_binding = contracts_mod.consume_normalized_task(
            method_id=_METHOD_ID,
            script_path=__file__,
            argv=sys.argv[1:],
        )
    except contracts_mod.NormalizedTaskBindingError as exc:
        die(f"Quickstart Task 绑定无效: {exc}", code=2)

    # ── 路径边界检查与证据加载（失败时输出 Result 信封） ──
    try:
        ...
        try:
            ...
            raise RuntimeError(f"无法读取证据文件: {e}") from e
        try:
            bundle: dict[str, Any] = json.loads(raw_bytes)
        except json.JSONDecodeError as e:
            raise RuntimeError(f"证据文件 JSON 解析失败: {e}") from e
        if not isinstance(bundle, dict):
            raise RuntimeError("证据包顶层必须是 JSON 对象")

        # ── 平台身份一致性：identity.platform 必须与 --platform 完全一致 ──
        identity = bundle.get("identity")
        if not isinstance(identity, dict):
            raise RuntimeError("证据包缺少 identity 对象")
        bundle_platform = identity.get("platform")
        if bundle_platform != platform:
            raise RuntimeError(
                f"证据包 identity.platform={bundle_platform!r} 与 --platform={platform!r} 不一致"
            )
    except Exception as exc:
        # 验证失败：构建 Result 信封并输出到 stdout 和 output_dir
        stage_id = f"runtime-audit-{getattr(args, 'maturity_level', 'unknown')}"
        # 尝试从 Quickstart Task 获取真实 run_id
        run_id = "run-runtime-audit-validation"
        try:
            ...
            pass
        error_obj = contracts_mod.build_error(
            error_code="RUNTIME_AUDIT.VALIDATION_FAILED",
            ...
            remediation="修正输入参数或证据包后重试",
        )
        failure_result = contracts_mod.build_result_envelope(
            run_id=run_id,
            stage_id=stage_id,
            execution_status="BLOCKED",
            summary=f"运行时审计预执行失败: {exc}",
            outputs=[],
            domain_results=[],
            evidence=[],
            missing_inputs=[],
        ...
        if quickstart_binding is not None:
            failure_result.setdefault("extensions", []).append(quickstart_binding)
            contracts_mod.publish_bound_failure_result(
                quickstart_binding, failure_result
            )
        # 直接调用只输出 stdout；有效 Quickstart Task 同时闭合方法输出目录。
        print(json.dumps(failure_result, ensure_ascii=False, sort_keys=True))
        sys.exit(2)
    ...
    if quickstart_binding is not None:
        result_envelope["extensions"] = [quickstart_binding]
