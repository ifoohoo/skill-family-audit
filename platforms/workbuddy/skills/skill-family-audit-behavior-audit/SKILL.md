...
---
<!-- platform-projection: platform=workbuddy; logical=skill-family-audit:behavior-audit; source=skills/skill-family-audit-behavior -->
...
最后原子发布领域结果与证据。公共 Task/Result 由 Foundation Quickstart profile 包装。

## 边界

- 只能在最低隔离画像内执行
- 不得绕过 Foundation Task、显式夹具或独占输出目录
- 不创建沙箱，不把测试目录冒充隔离证明
- 未知写入、目标项目漂移、超时或清理失败均失败关闭
- 命令执行边界可替换测试，测试不得访问真实用户配置
- 不安装或卸载目标插件；Kimi 仅支持人工安装或会话级 `--skills-dir`
