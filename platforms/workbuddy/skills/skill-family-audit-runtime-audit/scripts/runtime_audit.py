...
_SPEC = importlib.util.spec_from_file_location("_runtime_atomic_publish", _SHARED_PATH)
...
def atomic_write_bundle(output_dir: Path, file_map: dict[str, bytes]) -> None:
    # Domain cleanup: a stale empty output directory may be replaced, while a
    # non-empty one fails closed (rmdir raises ENOTEMPTY). The atomic
    # publication itself is delegated to the Foundation fixed-set mechanism.
    if output_dir.exists():
        output_dir.rmdir()
    try:
        _SHARED.publish_fixed_artifact_set(
            output_dir=str(output_dir),
            artifact_bytes=file_map,
            filenames=list(file_map.keys()),
            publisher_id="skill-family-audit-runtime",
        )
...
def main() -> int:
    try:
        if "conformance_check" not in sys.modules:
            script = next(
                (
                    root / relative
                    for root in Path(__file__).resolve().parents
            ...
            if script is None:
                raise RuntimeAuditError("FOUNDATION_TRANSPORT_MISSING")
