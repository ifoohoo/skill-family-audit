...
def _foundation_cli() -> Path:
    ...
    for ancestor in current.parents:
        ...
        if sibling.is_file():
            return sibling
    raise WorkflowError("FOUNDATION_UNAVAILABLE", "Foundation candidate mechanism CLI not found")
...
def _foundation(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(result, dict):
        raise WorkflowError("FOUNDATION_MECHANISM_FAILED", "Foundation returned non-object")
    return result
